...
- Before pushing, check `git diff --stat` or `git status --short`; also check `git log --oneline origin/<branch>..HEAD`. If there are unpushed commits that touch root `index.html`, `tools/`, `assets/`, or non-StarDict modules, stop and ask before pushing. After committing, check `git show --name-only HEAD` to confirm the commit did not touch other modules by accident.
